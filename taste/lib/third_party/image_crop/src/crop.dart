...
class CropState extends State<Crop> with TickerProviderStateMixin, Drag {
  ...
  Size get _boundaries => _surfaceKey.currentContext.size -
      const Offset(_kCropHandleSize, _kCropHandleSize) as Size;
  ...
  Rect _calculateDefaultArea({
    ...
    final aspect = widget.aspectRatio ??
        (imageWidth / imageHeight).clamp(
            widget.minAspect ?? (widget.square ? 1 : 0),
            widget.maxAspect ?? (widget.square ? 1 : 2));
    var width = aspect * imageHeight / imageWidth;
    var height =
        (imageWidth * viewWidth * width) / (imageHeight * viewHeight * aspect);
    ...
    height = 1 / width;
    width =
        (imageHeight * viewHeight * height) / (imageWidth * viewWidth / aspect);
  ...
  void _updateImage(ImageInfo imageInfo, bool synchronousCall) {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      setState(() {
        ...
        var viewWidth = _boundaries.width / (_image.width * _ratio);
        var viewHeight = _boundaries.height / (_image.height * _ratio);
        _area = _calculateDefaultArea(
          viewWidth: viewWidth,
        ...
        _scale = ((_image.height / _image.width).clamp(0, 1) *
                _boundaries.width /
                _boundaries.height)
  ...
  void _handleScaleEnd(ScaleEndDetails details) {
    ...
    final targetScale = _scale.clamp(_minimumScale, _maximumScale).toDouble();
